    ...
    <div>
        ...
        <v-container fluid class="pa-4">
            <v-card flat class="mb-6" rounded="lg">
                <v-card-text>
                    <v-form ref="form" v-model="valid" fast-fail @submit.prevent="save" class="form-container">
                        <v-row dense>
                            <!-- 左侧列：基本信息 -->
                            <v-col cols="12" md="6">
                                <v-text-field :rules="[rules.required, rules.checkNo]" v-model="formData.no" label="编号"
                                    density="compact" variant="outlined" required spellcheck="false"
                                    :disabled="isEditing || viewOnly" />
                                <v-text-field :rules="[rules.required]" v-model="formData.name" label="名称"
                                    density="compact" variant="outlined" required spellcheck="false" 
                                    :disabled="viewOnly" />
                                <v-text-field :rules="[rules.required, rules.validJdbcUrl]" v-model="formData.url"
                                    label="JDBC 地址" density="compact" variant="outlined" required spellcheck="false"
                                    :append-inner-icon="connFlag" :disabled="viewOnly" />
                            </v-col>

                            <!-- 右侧列：认证和驱动 -->
                            <v-col cols="12" md="6">
                                <v-text-field :rules="[rules.required]" v-model="formData.username" label="用户名"
                                    density="compact" variant="outlined" required spellcheck="false" 
                                    :disabled="viewOnly" />
                                <v-text-field v-model="formData.password" label="密码" density="compact"
                                    variant="outlined" :append-icon="showPassword ? 'mdi-eye' : 'mdi-eye-off'"
                                    :type="showPassword ? 'text' : 'password'"
                                    @click:append="showPassword = !showPassword" :disabled="viewOnly" />
                                <v-select :rules="[rules.required]" v-model="formData.driver" label="驱动类"
                                    :items="drivers" density="compact" variant="outlined" required 
                                    :disabled="viewOnly" />
                            </v-col>
                        </v-row>
                        ...
                        <v-row class="mt-4">
                            <v-col cols="12">
                                <div class="d-flex justify-space-between align-center">
                                    <v-btn v-if="!viewOnly" color="primary" variant="text" size="small" prepend-icon="mdi-connection"
                                        @click="testConn" :loading="testingConnection">
                                        测试连接
                                    </v-btn>
                                    <div v-else></div>
                                    ...
                                    <div class="d-flex gap-3">
                                        ...
                                        <v-btn v-if="!viewOnly" type="submit" :disabled="!valid" color="primary" variant="elevated"
                                            size="small" prepend-icon="mdi-content-save" :loading="loading">
                                            保存
                                        </v-btn>
                                    </div>
                                </div>
