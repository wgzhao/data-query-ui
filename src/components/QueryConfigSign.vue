  ...
  <v-card>
    ...
    <v-card-text>
      ...
      <div v-else>
        ...
        <v-data-table
          v-model="selectedQueryConfigs"
          :headers="queryConfigHeaders"
          :items="allQueryConfigs"
          :search="queryConfigSearch"
          item-value="selectId"
          density="compact"
          class="elevation-1"
          show-select
    ...
    <v-card-actions>
      <v-spacer />
      <v-btn color="secondary" variant="text"> 取消 </v-btn>
      <v-btn
        color="primary"
        variant="text"
        @click="saveQueryConfigRelations"
...
const saveQueryConfigRelations = async () => {
  ...
  try {
    const queryConfigIds = selectedQueryConfigs.value.map(
      item => item.selectId
    );
